...
function _addFullUrlsToImages(images) {
  return images.map((image) => ({
    ...image,
    highResUrl: storageService.storage.getFileUrl(image.highResUrl),
    thumbnailUrl: storageService.storage.getFileUrl(image.thumbnailUrl),
  }));
}
...
function _addFullUrlsToGroupCover(groups) {
  return groups.map((group) => ({
    ...group,
    latestImageUrl: storageService.storage.getFileUrl(group.latestImageUrl),
  }));
}
 ...
 * @returns {Promise<Object>} EXIF元数据对象
 * @throws {CustomError} 当EXIF读取失败时抛出错误
 */
async function extractImageMetadata(input) {
  try {
    // exiftool-vendored 库支持直接读取文件路径或Buffer
    return await exiftool.read(input);
  } catch (error) {
