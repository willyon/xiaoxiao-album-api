 ...
 * @LastEditTime: 2025-08-17 15:04:54
...
const { isImage, isDuplicate, formatImage, cleanupGeneratedFile, calculateImageHash, extractImageMetadata } = require("../services/imageService");
...
// 转换小图目录
const previewImageFolder = path.join(__dirname, "..", "..", process.env.PROCESSED_THUMBNAIL_IMAGE_DIR);
...
async function processAndSaveImage() {
  ...
  try {
    ...
    async function processImage(file) {
      ...
      // 获取图片哈希值
      const imageHash = await calculateImageHash(sourcePath);
      ...
      if (isAlreadyExist) {
        const duplicateFilePath = path.join(duplicateFolder, path.basename(sourcePath));
        ...
        try {
          //  格式化高质量大图
          var bigHighFilePath = path.join(highResFolder, `${imageHash}.${imgExtension}`);
          // 格式化小图
          var previewFilePath = path.join(previewImageFolder, `${imageHash}.${imgExtension}`);
          await Promise.all([
            formatImage([sourcePath, "-quality", "50", bigHighFilePath]),
            formatImage([sourcePath, "-quality", "50", "-resize", "600x", previewFilePath]),
          ]);
        } catch (err) {
          console.log(`图片文件 ${file} 格式转换失败: ${err}`);
          //将可能已转化成功的图片文件删除并跳出当前循环
          await cleanupGeneratedFile(bigHighFilePath);
          await cleanupGeneratedFile(previewFilePath);
          return;
        }
        // 获取图片元数据
        const exifData = await extractImageMetadata(sourcePath);
        let imageData = {
          originalUrl: path.join(`/${process.env.PROCESSED_ORIGINAL_IMAGE_DIR}`, `${file}`),
          highResUrl: path.join(`/${process.env.PROCESSED_HIGH_RES_IMAGE_DIR}`, `${imageHash}.${imgExtension}`),
          thumbnailUrl: path.join(`/${process.env.PROCESSED_THUMBNAIL_IMAGE_DIR}`, `${imageHash}.${imgExtension}`),
        ...
        try {
          ...
          const originalPath = path.join(originalFolder, path.basename(sourcePath));
