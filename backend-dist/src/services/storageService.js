...
class StorageService {
  ...
  async processAndStoreImage({
    ...
    logger.info({
      ...
      details: { sourceStorageKey, fileSize, dataLoadTime },
    });

    // 大文件特殊处理：降低质量以提升处理速度
    let finalQuality = quality;
    if (fileSize > FILE_SIZE_THRESHOLDS.LARGE) {
      finalQuality = Math.max(quality - 10, 50); // 大文件降低质量，但不低于50
      logger.info({
        message: "Large file detected, reducing quality for faster processing",
        details: { fileSize, originalQuality: quality, finalQuality },
      });
    }
    ...
    pipeline = _applyEncoderByExt(pipeline, extension, finalQuality, fileSize, finalOptions);
