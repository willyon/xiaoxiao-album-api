...
const imageUploadQueue = new Queue(process.env.IMAGE_UPLOAD_QUEUE_NAME, {
  connection,
  defaultJobOptions: {
    attempts: Number(process.env.IMAGE_UPLOAD_JOB_ATTEMPTS || 5), //最多尝试次数（包括第一次执行）
    ...
    backoff: {
      ...
      delay: Number(process.env.IMAGE_UPLOAD_JOB_BACKOFF_DELAY || 1000),
    },
...
function closeImageUploadQueue() {
  return Promise.resolve()
    .then(() => imageUploadQueue.close())
    .catch(() => {})
    .then(() => connection.quit?.())
    .catch(() => {});
}

module.exports = { imageUploadQueue, queueConnection: connection, closeImageUploadQueue };
