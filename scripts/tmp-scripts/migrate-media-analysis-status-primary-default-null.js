/*
 * 迁移：去掉 media.analysis_status_primary 的列级 DEFAULT（历史常为 DEFAULT 'pending'），
 * 使新建行在未写该列时为 SQL NULL，与 analysis_status_cloud / initTableModel / insertMedia 约定一致。
 *
 * - 若列不存在：ADD COLUMN analysis_status_primary TEXT（无 DEFAULT）
 * - 若列存在且 PRAGMA table_info 中 dflt_value 已为 NULL：跳过
 * - 若列存在且仍有 DEFAULT：sqlite_master 打补丁 → media_new → 复制 → 换表 → 重建索引
 *
 * 用法（在 xiaoxiao-project-service 根目录）:
 *   node scripts/tmp-scripts/migrate-media-analysis-status-primary-default-null.js
 *
 * 建议先备份 database；可重复执行。
 */
...
function getPrimaryColumnInfo() {
  const rows = db.prepare("PRAGMA table_info(media)").all();
  return rows.find((c) => c.name === "analysis_status_primary") || null;
}
...
function buildMediaNewCreateSql(originalCreateSql) {
  ...
  const patterns = [
    /\banalysis_status_primary\s+TEXT\s+DEFAULT\s+'pending'/gi,
    /\banalysis_status_primary\s+TEXT\s+DEFAULT\s+"pending"/gi,
    /\banalysis_status_primary\s+TEXT\s+DEFAULT\s+\(NULL\)/gi,
  ];

  let body = originalCreateSql;
  let changed = false;
  for (const re of patterns) {
    const next = body.replace(re, "analysis_status_primary TEXT");
    if (next !== body) {
      body = next;
      changed = true;
      break;
  ...
  if (!changed) {
    const loose = body.replace(/\banalysis_status_primary\s+TEXT\s+DEFAULT\s+[^,)]+/gi, "analysis_status_primary TEXT");
    if (loose !== body) {
      body = loose;
      changed = true;
    }
...
function rebuildMediaTable() {
  ...
  if (!createMediaNewSql) {
    throw new Error(
      "sqlite_master 中的 CREATE TABLE 无法自动去除 analysis_status_primary 的 DEFAULT，请检查列定义或手工迁移",
    );
  }
  ...
  console.log("开始重建 media 表：去除 analysis_status_primary 的 DEFAULT …");
  ...
  try {
    ...
    db.exec("COMMIT");
    console.log("✅ 已完成：analysis_status_primary 无列级 DEFAULT，新建行可为 NULL");
  } catch (err) {
    db.exec("ROLLBACK");
    throw err;
  } finally {
...
function migrate() {
  ...
  const col = getPrimaryColumnInfo();

  if (!col) {
    db.prepare("ALTER TABLE media ADD COLUMN analysis_status_primary TEXT").run();
    console.log("✅ 已新增列 media.analysis_status_primary TEXT（无 DEFAULT）");
    return;
  }

  if (col.dflt_value == null || col.dflt_value === "") {
    console.log("ℹ️ media.analysis_status_primary 已无列级 DEFAULT，跳过");
    return;
  }

  console.log(`ℹ️ 检测到 analysis_status_primary 的 DEFAULT=${JSON.stringify(col.dflt_value)}，执行重建 …`);
  rebuildMediaTable();
}
