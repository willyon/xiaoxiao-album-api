/*
 * 归档：从旧 images 表移除 color_theme 列及其索引；重建后的 images_new 同时不含 storage_type（与当前「仅存 key、存储方式由 .env」一致）。
 ...
 *
 * @Usage: node scripts/tmp-scripts/migrate-images-remove-color-theme.js
 */
...
// 新表结构（不含 color_theme、不含 storage_type）
...
// 插入时排除 color_theme；不复制 storage_type（新表无该列）
const INSERT_SQL = `
...
function migrate() {
  ...
  const hasColorTheme = info.some((col) => col.name === "color_theme");
  if (!hasColorTheme) {
    console.log("images 表已无 color_theme 列，跳过迁移");
    return;
  }

  console.log("开始迁移：移除 images.color_theme...");
  ...
  try {
    ...
    // 3. 创建新表（无 color_theme、无 storage_type）
    ...
    // 7. 重建索引（不含 idx_images_user_color_theme）
    const indexes = [
      ...
      "CREATE INDEX IF NOT EXISTS idx_images_user_age_flags ON images(user_id, has_young, has_adult)",
    ...
    db.exec("COMMIT");
    console.log("✅ 迁移完成：images 已移除 color_theme；新表无 storage_type，且无 idx_images_user_color_theme / idx_images_user_storage_creation");
  } catch (err) {
    db.exec("ROLLBACK");
