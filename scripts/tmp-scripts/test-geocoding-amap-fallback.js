...
async function main() {
  ...
  await runCase("1. 请求超时 → 降级", "timeout", LAT_TW, LNG_TW, (r) => {
    ...
    if (r.mapRegeoStatus !== "failed") throw new Error("线上失败应标记 failed");
    if (!loc || !loc.province) throw new Error("应命中本地省/区划");
    if (loc.province !== "台湾" && loc.formattedAddress !== "台湾") throw new Error(`意外结果: ${JSON.stringify(loc)}`);
  });

  await runCase("2. 网络错误 → 降级", "neterr", LAT_TW, LNG_TW, (r) => {
    if (r.mapRegeoStatus !== "failed") throw new Error("应标记 failed");
    if (!r.location || !r.location.province) throw new Error("应命中本地");
  });
  ...
  await runCase("6. 高德成功 → 不降级（用 mock 结果）", "success", LAT_TW, LNG_TW, (r) => {
    const loc = r.location;
    if (r.mapRegeoStatus !== "success") throw new Error("高德成功应标记 success");
    if (!loc || loc.formattedAddress !== "模拟高德成功地址") throw new Error("应直接返回高德解析结果");
    if (loc.province !== "台湾省") throw new Error("应保留高德 province");
  });

  await runCase("7. 台湾降级命中后；境外点降级走全球（巴黎）", "status0", LAT_FR, LNG_FR, (r) => {
    if (r.mapRegeoStatus !== "failed") throw new Error("应标记 failed");
    const loc = r.location;
    if (!loc || !loc.country) throw new Error("全球应返回国家");
    if (loc.country !== "法国") throw new Error(`期望法国，得到 ${loc.country}`);
  });
