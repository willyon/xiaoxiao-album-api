/*
 * @Description: 清理非用户核心数据（Redis、除 users 外的数据库表、localStorage 中图片/视频）
...
function clearLocalStorageMedia() {
  ...
  console.log(
    `  🎉 localStorage 清理完成：扫描 ${result.scannedFiles} 个文件，删除 ${result.deletedFiles} 个图片/视频文件`,
  );
  return result;
}
...
async function main() {
  console.log("==============================================");
  console.log("🧹 开始清理：Redis + DB(保留 users) + localStorage媒体");
  console.log("==============================================");
  ...
  console.log(`- localStorage: 删除 ${storageResult.deletedFiles} 个文件`);
}
