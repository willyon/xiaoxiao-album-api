 ...
 * 典型场景：回收站静默恢复后只写了 uploadedCount、未写 ingestDoneCount，导致媒体处理页一直 0/N。
 *
 * 规则（与 uploadProgressSnapshot.computeMediaStageDone 一致）：
 *   mediaProcessed = ingestDoneCount + ingestErrorCount + workerSkippedCount
 *   若 uploadedCount > mediaProcessed，则将差额补到 ingestDoneCount（视为应已完成流水线但未计数）。
 *
 * 用法:
...
async function repairOneSession(redis, sessionKey, apply) {
  ...
  const ingestDoneCount = toInt(data.ingestDoneCount);
  const ingestErrorCount = toInt(data.ingestErrorCount);
  const workerSkippedCount = toInt(data.workerSkippedCount);

  const mediaProcessed = ingestDoneCount + ingestErrorCount + workerSkippedCount;
  ...
  if (!apply) {
    return {
      ...
      dryRun: true,
      uploadedCount,
      ingestDoneCount: mediaProcessed,
      deficit,
      wouldAddToIngestDoneCount: deficit,
    };
  }

  await redis.hincrby(sessionKey, "ingestDoneCount", deficit);
...
说明:
  扫描 upload:session:* 主 Hash，若 uploadedCount > ingestDoneCount+ingestErrorCount+workerSkippedCount，
  则把差额补到 ingestDoneCount（与前端「基础处理」进度一致）。
...
async function main() {
  ...
  for (const key of keys) {
    ...
    if (result.dryRun) {
      console.log(
        `  [dry-run] ${result.sessionId}  uploaded=${result.uploadedCount} ingestDoneCount=${result.ingestDoneCount}  → 将 ingestDoneCount += ${result.deficit}`,
      );
      fixed++;
    } else {
      console.log(`  [done]  ${result.sessionId}  ingestDoneCount += ${result.deficitApplied}`);
      fixed++;
    }
  }
